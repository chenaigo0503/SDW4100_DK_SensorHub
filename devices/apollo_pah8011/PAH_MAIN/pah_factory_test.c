...
typedef enum
{
    ...
    factory_test_INT_Short,
    factory_test_power_noise,
	factory_test_led_target_sample_ledoff,
    factory_test_num,
} factory_test_e;
...
static bool demo_factory_test(factory_test_e factory_test,bool expo_en,uint8_t expo_ch_b,uint8_t expo_ch_c)
{
	...
	case factory_test_INT_Short:
        debug_printf("===== Factory test: INT1&2_Short ===== \n");
        ret = pah_verify_INT_short();
        if (PAH_FAILED(ret))
        Error_Handler();
        temp=1;
        //return true;
        //NOT REACHED
        break;
	case factory_test_led_target_sample_ledoff:
        debug_printf("===== Factory test: LED target sample led off ===== \n");
        ret = pah_verify_led_target_sample_ledoff_read_adc_data(&adc_data);
        if (PAH_FAILED(ret))
            Error_Handler();
        break;
