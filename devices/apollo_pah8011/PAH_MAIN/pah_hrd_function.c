...
volatile bool        has_interrupt_button = false;
volatile bool        has_interrupt_pah = false;
volatile uint64_t    interrupt_pah_timestamp = 0;
/*============================================================================
...
static void report_fifo_data_hr(uint64_t timestamp, uint8_t *fifo_data, uint32_t fifo_data_num_per_ch, uint32_t ch_num, bool is_touched)
{
    ...
    if (timestamp < _state.last_report_time)
        _state.pxialg_data.time = 1100;
    else
...
static bool hr_algorithm_calculate(pah8series_data_t *pxialg_data, uint32_t ch_num)
{
    ...
    if (!_state.pxialg_has_init)
    {
		...
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_FLAG_LIMIT_HR_UB, 1);
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_LIMIT_HR_UB, 220);
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_FLAG_LIMIT_HR_LB, 1);
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_LIMIT_HR_LB, 110);
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_FLAG_EN_ONHAND_RECONIZATION, 1);
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_FLAG_PREDICTION_FEEDBACK, 1);
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_FLAG_PREDICTION_SMOOTH, 1);
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_SMOOTH_TRUST_LEVEL_TH, 1);
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_FLAG_MEMS_3_SEP, 1);
		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_FLAG_EN_CHANNEL_QUALITY, 1);

		algorithm_param_set(PAH8SERIES_PARAM_IDX_SET_FALG_IMP, 1);

        _state.pxialg_has_init = true;
    }
    // Calculate Heart Rate
    hr_stat = pah8series_entrance(pxialg_data);
