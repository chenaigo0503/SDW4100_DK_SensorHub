...
FAIL:
    debug_printf("pah_global_reset() failed. ret = %d \n", ret);
    return ret;
}
...
pah_ret pah_verify_INT_short(void)
{
    pah_ret ret = pah_err_unknown;
	ret = pah_8011_startup();
    if (PAH_FAILED(ret))
        goto FAIL;
    // write settings
    ret = pah_8011_write_reg_array(pah8011_verify_int_short_array, PAH_ARRAY_SIZE(pah8011_verify_int_short_array));
    if (PAH_FAILED(ret))
        goto FAIL;
    return pah_success;

FAIL:
    debug_printf("pah_verify_INT_Short failed. ret = %d \n", ret);
    return ret;
}
...
pah_ret pah_verify_led_target_sample_ledoff_read_adc_data(pah_verify_adc_data_s *adc_data)
{
    pah_ret ret = pah_err_unknown;

    debug_printf("pah_verify_led_target_sample_ledoff_read_adc_data() \n");
    ...
    ret = _pah8011_write_settings_and_read_adc_data(pah8011_verify_led_brightness_led_off_array, PAH_ARRAY_SIZE(pah8011_verify_led_brightness_led_off_array), adc_data,&expo_time_data_null);
    if (PAH_FAILED(ret))
        goto FAIL;

    // a
#if defined(PAH_LED_TARGET_SAMPLE_LEDOFF_CH_A_MIN)
    adc_data->ch_adc_data[pah_verify_ch_a].min_enabled_verify = true;
    adc_data->ch_adc_data[pah_verify_ch_a].min = PAH_LED_TARGET_SAMPLE_LEDOFF_CH_A_MIN;
#endif
#if defined(PAH_LED_TARGET_SAMPLE_LEDOFF_CH_A_MAX)
    adc_data->ch_adc_data[pah_verify_ch_a].max_enabled_verify = true;
    adc_data->ch_adc_data[pah_verify_ch_a].max = PAH_LED_TARGET_SAMPLE_LEDOFF_CH_A_MAX;
#endif

    // b
#if defined(PAH_LED_TARGET_SAMPLE_LEDOFF_CH_B_MIN)
    adc_data->ch_adc_data[pah_verify_ch_b].min_enabled_verify = true;
    adc_data->ch_adc_data[pah_verify_ch_b].min = PAH_LED_TARGET_SAMPLE_LEDOFF_CH_B_MIN;
#endif
#if defined(PAH_LED_TARGET_SAMPLE_LEDOFF_CH_B_MAX)
    adc_data->ch_adc_data[pah_verify_ch_b].max_enabled_verify = true;
    adc_data->ch_adc_data[pah_verify_ch_b].max = PAH_LED_TARGET_SAMPLE_LEDOFF_CH_B_MAX;
#endif

    // c
#if defined(PAH_LED_TARGET_SAMPLE_LEDOFF_CH_C_MIN)
    adc_data->ch_adc_data[pah_verify_ch_c].min_enabled_verify = true;
    adc_data->ch_adc_data[pah_verify_ch_c].min = PAH_LED_TARGET_SAMPLE_LEDOFF_CH_C_MIN;
#endif
#if defined(PAH_LED_TARGET_SAMPLE_LEDOFF_CH_C_MAX)
    adc_data->ch_adc_data[pah_verify_ch_c].max_enabled_verify = true;
    adc_data->ch_adc_data[pah_verify_ch_c].max = PAH_LED_TARGET_SAMPLE_LEDOFF_CH_C_MAX;
#endif
   // t
#if defined(PAH_LED_TARGET_SAMPLE_LEDOFF_CH_T_MIN)
    adc_data->ch_adc_data[pah_verify_ch_t].min_enabled_verify = true;
    adc_data->ch_adc_data[pah_verify_ch_t].min = PAH_LED_TARGET_SAMPLE_LEDOFF_CH_T_MIN;
#endif
#if defined(PAH_LED_TARGET_SAMPLE_LEDOFF_CH_T_MAX)
    adc_data->ch_adc_data[pah_verify_ch_t].max_enabled_verify = true;
    adc_data->ch_adc_data[pah_verify_ch_t].max = PAH_LED_TARGET_SAMPLE_LEDOFF_CH_T_MAX;
#endif
    return pah_success;

FAIL:
    debug_printf("pah_verify_led_target_sample_ledoff_read_adc_data() failed. ret = %d \n", ret);
    return ret;
}
